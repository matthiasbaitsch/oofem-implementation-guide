...
In this section, for each of the three analysis phases, a software design is presented and you are guided step by step through the implementation. Of course, there are many other paths to get to the same endgoal. Feel free to go your own way if you are familiar with programming and finite element analysis.

One important to choice to make is that of the spatial dimension of the structure: Should the software analyze plane truss structures or spatial truss structures. For trusses, there is not much of difference in theory and implementation. Mostly everything is the same except that position and displacement vectors (and some others) have either two or three entries. However, for the visualization of a spatial structure you need a suitable 3D graphics library. While in Python you can use PyVista for 3D visualization, there is not easy to use 3D library for C#. 
...
1. In order to work with matrices and vectors, you can use the [Math.NET Numerics](https://numerics.mathdotnet.com){target="_blank"} package. Add the lines
  ...
  to the `oofem.csproj` file. The package reference include the package, the following two lines simplify usage.
...
- For simplicity, all attributes are public. We use the capital first letters for attributes and methods, please adapt to the programming style of you language of choice.
...
- The `Constraint` and `Force` classes just holders of an array. An alternative would have been to directly introduce a boolean and a double array to the node class for constraints and forces.
...
```{.csharp}
 {{< include oofem-csharp/oofem-01/Program.cs >}}
```
...
Program the `Visualizer` class in the UML class diagram in @fig-class-diagram-1. 

::: {.columns}
::: {.column width=45%}
```{.csharp}
 {{< include oofem-csharp/oofem-visualizer-01/Program.cs >}}
```
:::
::: {.column width=5%}
:::
...
The additional methods needed for the processing step are shown inf @fig-class-diagram-2. We now implement them step by step.
...
$$
\mathbf{d}^e = \frac{1}{l} (\mathbf{X}^{e2} - \mathbf{X}^{e1})
$$ 
...
In order to test the methods, add the length to the element output and compute the stiffness matrix for element 3. Below the testing code
...
```{.csharp include="oofem-csharp/oofem-02/Program.cs" start-line=20 end-line=30}
```
...
1. Loop over each element of the structure
    - Compute element stiffness matrix $\mathbf{K}^e$ and store element DOFs in a variable `dofs`
    - Use two nested loops for the element DOFs `i` and `j`
    - The corresponding system DOFs are `I = dofs[i]` and `J = dofs[j]`
    - If `I` and `J` both are not -1, add the local stiffness matrix entry $K^e_{ij}$ to the system matrix $K_{IJ}$
...
as reference. The final result can be verified by the following code and output.

::: {.panel-tabset}

## Implementation in C\#

```{.csharp include="oofem-csharp/oofem-02/Program.cs" start-line=33 end-line=37}
```
...
```raw-output
Node 0 has displacement [0, 0]
Node 1 has displacement [0.0048, 0]
Node 2 has displacement [0.0162, 0.0027]
```

## Postprocessing

With the nodal displacements available, the last step is to derive the quantity of interest for a truss analysis - the normal force in each element - and to present the results, both as formatted text output and as pictures of the deformed structure and the element normal forces. @fig-class-diagram-3 shows the additional methods needed for this step, building on the classes already implemented.
...
with the same layout as the transformation vector $\mathbf{t}^e$ introduced earlier, the normal force is
...
```{.csharp include="oofem-csharp/oofem-02/Program.cs" start-line=40 end-line=40}
```
...
```{.csharp include="oofem-csharp/oofem-02/Program.cs" start-line=42 end-line=42}
```
...
```{.csharp include="oofem-csharp/oofem-visualizer-02/Program.cs" start-line=19 end-line=23}
```
...
```{.csharp include="oofem-csharp/oofem-visualizer-02/Program.cs" start-line=25 end-line=28}
```
