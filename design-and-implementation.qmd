...
With the nodal displacements available, the last step is to compute the normal force in each element and to present the results, both as formatted text output and as pictures of the deformed structure and the element normal forces. @fig-class-diagram-3 shows the additional methods needed for this step, building on the classes already implemented.
...
with the same layout as the transformation vector $\mathbf{t}^e$ introduced earlier, the elongation is $(\mathbf{t}^e)^T \mathbf{u}^e$ and the normal force is
